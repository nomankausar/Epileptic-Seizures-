...
def main():
    for fname in os.listdir(data_dir):
        ...
        # compute std
        std_ez  = EZ.std(axis=0)
        std_non = NON_EZ.std(axis=0)
        ...
        # Annotate selected q-points
        q_points = [-20, -15, -10, -5, 0, 5, 10, 15, 20]
        diff_data = []

        for i, q_val in enumerate(q_points):
            idx = np.argmin(np.abs(q_values - q_val))
            ez_m, ez_s = mean_ez[idx], std_ez[idx]
            non_m, non_s = mean_non[idx], std_non[idx]
            diff_mean = ez_m - non_m
            diff_std  = ez_s - non_s
            y_pos = max(ez_m, non_m) + 0.05
            annotation = f"Δμ={diff_mean:.2f}\nΔσ={diff_std:.2f}"
            plt.annotate(
                annotation,
                xy=(q_val, y_pos),
                xytext=(q_val, y_pos + 0.03),
                textcoords='data',
                ha='center',
                fontsize=7,
                color='black',
        ...
        # Export selected q-points differences to CSV
        diff_df = pd.DataFrame(diff_data)
        diff_df.to_csv(os.path.join(data_dir, f"{base}_differences.csv"), index=False)
        print(f"→ Saved Δmean/Δstd values to {base}_differences.csv")

        # Export full q-range differences (401 points) to CSV
        full_diff_data = {
            'q': q_values,
            'EZ_mean': mean_ez,
            'NonEZ_mean': mean_non,
            'Mean_Diff': mean_ez - mean_non,
            'EZ_std': std_ez,
            'NonEZ_std': std_non,
            'Std_Diff': std_ez - std_non
        }
        full_df = pd.DataFrame(full_diff_data)
        full_df.to_csv(os.path.join(data_dir, f"{base}_full_differences.csv"), index=False)
        print(f"→ Saved full q-range Δmean/Δstd values to {base}_full_differences.csv")
